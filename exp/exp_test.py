...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_criterion(self):
        if self.args.loss == "MSE":
            ...
            criterion = FrequencyDomainL1Loss(alpha=0.3, freq_mode='log').to(self.args.device)
        return criterion
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            # -------------------------------
            # 4. 在每个 epoch 结束时打印平均迭代时间
            # -------------------------------
            avg_iter_time = np.mean(iter_times) if len(iter_times) > 0 else 0.0
            print(f"Epoch {epoch+1} average iteration time: {avg_iter_time:.4f} s")
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                batch_y_sliced = batch_y[:, -self.args.pred_len:, :].to(self.device)
                outputs = outputs.detach().cpu().numpy()
                batch_y_np = batch_y_sliced.detach().cpu().numpy()

                if test_data.scale and self.args.inverse:
                    shape = batch_y_np.shape
                    if outputs.shape[-1] != batch_y_np.shape[-1]:
                        outputs = np.tile(outputs, [1, 1, int(batch_y_np.shape[-1] / outputs.shape[-1])])
